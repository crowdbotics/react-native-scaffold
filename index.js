 ...
 *
 * Run it in your app's repository with: npx crowdbotics/react-native-scaffold
 */
...
function setupLocalScaffoldRepo(version) {
  if (fs.existsSync(path.join(userdir, config.upgrade.build.scaffoldRepoDir))) {
    section("Reusing local clone of scaffold repo");
    return;
  }
  section("Cloning scaffold repo");
  spawnSync("mkdir", ["-p", "build"], { cwd: userdir });
  spawnSync("git", ["init", config.upgrade.build.scaffoldRepoDir], {
    cwd: userdir
  });
  spawnSync(
    "git",
    ["remote", "add", "origin", config.constants.REACT_NATIVE_SCAFFOLD_REPO_ORIGIN],
    {
...
function setupCookiecutter(context, version) {
  ...
  const run = spawnSync(
    ...
    [
      "run",
      "cookiecutter",
      path.join(userdir, config.upgrade.build.scaffoldRepoDir, "dist", "cookie"),
      "--config-file",
      path.join(
        userdir,
        config.upgrade.build.scaffoldRepoDir,
...
const upgrade = (version) => {
  const performUpgrade = () => {
    ...
    stashSave();
    setupLocalScaffoldRepo(version);
